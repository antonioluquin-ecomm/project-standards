...
**Proyectos:** Marketplace Portal · Commerce Hub · VTEX Control Center · VTEX Bookmarklets · Adidas Order Control Center · Herramientas internas
...
### 10.1 Tipos de commit y cuándo usarlos

| Prefijo | Uso | Ejemplo |
|---------|-----|---------|
| `feat:` | Nueva funcionalidad visible | `feat: agregar sidebar de navegación` |
| `fix:` | Corrección de bug | `fix: corregir filtro de búsqueda con acentos` |
| `style:` | Cambios visuales sin lógica | `style: ajustar padding del sidebar en mobile` |
| `refactor:` | Cambio interno sin impacto visible | `refactor: extraer normalizeText a app.js` |
| `docs:` | Solo documentación | `docs: actualizar README con estructura de carpetas` |
| `chore:` | Bump de versión, configuración | `chore: bump versión a 1.5.0` |
| `fix(security):` | Corrección de seguridad | `fix(security): escapar HTML en renderizado de datos API` |

### 10.2 Reglas de commits

- **Un propósito por commit.** No mezclar una feature con un fix con un cambio de estilo.
- **Mensaje en infinitivo**, sin punto final: `agregar módulo` no `se agregó el módulo`.
- **Cuerpo opcional pero valioso** para commits de alto impacto: explicar el *por qué*, no el *qué*.
- **Incluir Co-Authored-By** cuando el commit es generado principalmente por una IA:
  ```
  Co-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>
  ```
- **No commitear archivos sensibles**: `.env`, credenciales, tokens, datos personales de usuarios.
- **No usar `--no-verify`** salvo que un hook falle por razón externa al código (y documentarlo).

### 10.3 Branches

```
main              → producción, siempre deployable
feature/nombre    → feature nueva (desde main, merge a main)
fix/descripcion   → bugfix (desde main, merge a main)
hotfix/descripcion→ fix urgente en producción
```

En proyectos personales o de equipo pequeño donde se trabaja directo en `main`: aceptable, pero igualmente seguir la disciplina de commits atómicos.

### 10.4 Lo que la IA no debe hacer en Git sin autorización

- `git push` — siempre pedir confirmación
- `git push --force` — nunca, salvo excepción documentada
- `git reset --hard` — confirmar antes, describir qué se va a perder
- `git branch -D` — confirmar antes
- Modificar `.gitignore` para incluir archivos que deberían estar ignorados permanentemente (discutirlo primero)
